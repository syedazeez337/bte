//! Linux terminal backend implementation
//!
//! Uses the nix crate for POSIX-compliant terminal operations.
...
impl TerminalProcess for LinuxTerminalProcess {
    ...
    fn resize(&mut self, cols: u16, rows: u16) -> Result<(), std::io::Error> {
        ...
        unsafe {
            let ret = libc::ioctl(fd.as_raw_fd(), libc::TIOCSWINSZ as _, &winsize);
            if ret < 0 {
...
impl Drop for LinuxTerminalProcess {
    fn drop(&mut self) {
        self.master_fd.take();
    }
}
